...
HerdOS workers run as GitHub Actions on self-hosted runners. Self-hosted runners are required because workers need an AI agent (Claude Code, OpenCode, or Codex) installed, and because GitHub-hosted runners don't support `workflow_dispatch` chaining with custom tools.
...
> **AI provider auth env vars (Claude, OpenCode, Codex) belong only in the runner's `.env`, not in GitHub Actions secrets.** The worker workflow's `env:` block would override container env unconditionally — surfacing these from secrets either no-ops or shadow-overrides your `.env` value at the step level. The runner container reads them from `.env` at startup (via `docker-compose`'s automatic `env_file` or `docker run --env-file .env`); the workflow inherits them from the runner process.
...
For a personal ChatGPT subscription (Plus/Pro/Team/Edu), authenticate **directly against the already-running worker container**. The CLI writes `~/.codex/auth.json` into the persistent `codex-auth` docker volume (mounted at `/home/runner/.codex`), which is the **source of truth** for subscription auth:

```bash
sudo docker exec -it -u runner <worker-container> codex login --device-auth
```

Complete the device-auth flow in your browser when prompted. On success, the Codex CLI writes `~/.codex/auth.json` into the `codex-auth` volume; thereafter Codex refreshes the OAuth token in place inside that volume. Because all worker replicas share the one `codex-auth` volume — `herd init` renders a single volume mounted at `/home/runner/.codex` for exactly this reason — a **single login covers every worker**. The background keepalive (see [Keepalive](#keepalive) below) keeps the idle chain warm so it doesn't expire between batches.

> **Rate-limit caveat**: ChatGPT Plus/Pro/Team rate limits are **per account**. Running additional worker replicas does **not** multiply your LLM-call throughput — they all share the one `auth.json` and the one account. Extra workers help only when tasks are wall-clock-bound on non-LLM work (builds, tests, I/O), not when they're bottlenecked on model calls.
...
The OAuth chain can break server-side — a password change, "log out everywhere", or an OpenAI session expiry will invalidate it. When that happens herd surfaces an auth error from the failing worker. To recover, write a fresh `auth.json` into the volume and restart:

1. Re-run the login against a running worker (or use the `docker run` fallback above if none is up):

   ```bash
   sudo docker exec -it -u runner <worker-container> codex login --device-auth
   ```

2. Restart the runner(s).

The refreshed `auth.json` in the `codex-auth` volume is the source of truth — once it's rewritten, every worker picks it up.
...
All worker replicas share the single `codex-auth` volume and therefore the one `auth.json`, which is what the second warning cautions against. herd makes this safe in practice through Codex's self-healing refresh behavior: refreshes fire ~5 minutes before the ~1-hour access token expires, so a refresh-token race fails at most one worker at a time, and that worker recovers on its next invocation by reading the rotated tokens already in the shared `auth.json`. For sustained 24/7 high-concurrency workloads where you want to honor the warning strictly, use ChatGPT Enterprise (`CODEX_ACCESS_TOKEN`, Path A), which has no shared per-runner auth state.
...
```dockerfile
FROM ghcr.io/herd-os/herd-runner-base:vX.Y.Z
USER root
...
`docker-compose.herd.yml` is regenerated by `herd init`. To add project-specific configuration (non-secret build args, extra volumes, environment variables), create a `docker-compose.herd.override.yml` using normal Docker Compose YAML:
...
The base image is pinned to the herd version that generated `Dockerfile.herd_runner` (for example, `:vX.Y.Z`). The pin is refreshed when you re-run `herd init` from a newer herd binary: if the existing `FROM` line still references the legacy local base (`FROM herd-runner-base[:tag]`), it is auto-migrated to the version-pinned `ghcr.io/herd-os/herd-runner-base:<herd-version>` reference (see [Migrating from the local base image](#migrating-from-the-local-base-image)). `FROM` lines that already point at `ghcr.io` or use a custom base are left untouched, so re-running `herd init` will not bump an already-published `ghcr.io` reference for you — refresh those by editing `Dockerfile.herd_runner` yourself. Dev builds (an empty or `dev` version) pin to `:latest`, because a `:dev` tag does not exist in GHCR and would fail to pull.
...
### Private dependencies during image builds

If `Dockerfile.herd_runner` installs private dependencies while building the runner image, list the required credential names under `image_publish.build_secrets`:
...
This is distinct from `workers.extra_env`, which affects worker task execution only. `workers.extra_env` does not pass secrets into `herd image build` or the generated image publish workflow.

For GitHub publishing, create a repository or organization Actions secret with the same name, then re-run `herd init` to regenerate `.github/workflows/herd-publish-runner.yml`. The generated workflow maps each configured name to `${{ secrets.NAME }}` and passes it to Docker as a BuildKit secret.

For local builds, export the same variable in your shell before running `herd image build`:
...
Herd reads configured build secrets from the caller environment for local image builds. It does not read GitHub secrets or `.env`, and it fails before Docker runs if a configured variable is missing.

Use BuildKit secret mounts in `Dockerfile.herd_runner`, not Docker `ARG` values. Herd normalizes secret names before passing them to Docker; for example, `BUNDLE_RUBYGEMS__PKG__GITHUB__COM` becomes the BuildKit secret ID `bundle_rubygems_pkg_github_com`.
...
RUN --mount=type=secret,id=bundle_rubygems_pkg_github_com \
    if [ -f /run/secrets/bundle_rubygems_pkg_github_com ]; then \
      export BUNDLE_RUBYGEMS__PKG__GITHUB__COM="$(cat /run/secrets/bundle_rubygems_pkg_github_com)"; \
    fi; \
...
### Checkout depth for review

The workflow files generated by `herd init` use `actions/checkout` with `fetch-depth: 0` for worker and integrator jobs that need local git history. Keep that full-history checkout when customizing the workflows. It lets HerdOS prepare review input from local git diffs before falling back to GitHub raw diff or files API metadata for large PRs.
