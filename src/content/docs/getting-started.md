...
- **Worker consolidation merge conflict** — When a worker branch conflicts while the Integrator is merging it into the batch branch, `integrator.on_conflict` controls the response. With `dispatch-resolver`, the Integrator creates a conflict-resolution issue and dispatches a worker automatically; with `notify`, it posts a comment for manual resolution. The number of resolver attempts is limited by `max_conflict_resolution_attempts`; when that budget is exhausted, the batch enters cascade-failed state and the `herd/cascade-failed` label is applied to the batch PR — see [design/execution.md → When cascades fail](design/execution.md#when-cascades-fail). If fresh GitHub PR metadata later reports the PR clean and mergeable, HerdOS automatically removes a stale `herd/cascade-failed` label; humans may still remove it manually after resolving the underlying conflict.
- **Batch PR merge conflict** — If an already-open batch PR becomes conflicted with its base branch, post `/herd resolve-conflicts` on the batch PR. HerdOS checks live GitHub mergeability, waits briefly when mergeability is unknown, and creates a focused conflict-resolution issue only when the PR currently conflicts with base. Use generic `/herd fix` for normal code fixes; it still includes PR conversation history for worker context.
- **Review safety valve** — If a single agent review finds more than 10 issues, fix workers are not created (to prevent runaway invocations). For chunked reviews, duplicate findings are deduplicated before the final PR comment and before fix workers are dispatched. The PR is flagged for manual intervention when the safety valve still applies.
- **Review non-convergence** — If several completed review-fix cycles do not reduce repeated package/root-cause clusters, HerdOS can optionally synthesize the dominant architectural/root-cause pattern, then creates and dispatches one strategy-level fix issue instead of asking for another `/herd fix`.
...
| `/herd resolve-conflicts` | Batch PR | Checks live GitHub mergeability, waits briefly if mergeability is unknown, and creates a focused conflict-resolution issue only when the PR currently conflicts with its base branch. The resolver issue avoids full PR conversation history. |
| `/herd resolve-conflicts <context>` | Batch PR | Same as above, with extra context included in the focused resolver issue. |
| `/herd retry <issue-number>` | Any issue/PR | Re-dispatches a failed issue |
| `/herd review` | Any PR | Triggers an automated chunked agent review of the PR and posts one aggregated result. Duplicate chunk findings are deduplicated before the comment and, on batch PRs, before fix workers are dispatched. Review comments may include raw, deduped, or stale-filter counts when filtering changed the result. On batch PRs, this manually requests a fresh pass for the current head even if an automatic review already approved it; findings create fix issues and dispatch workers. On non-batch PRs, only the severity-classified findings comment is posted. Non-HerdOS user comments on the PR are passed to the agent as feedback (e.g., to mark findings as false positives — see [User Feedback in Reviews](#user-feedback-in-reviews)). |
